# Test Plan Template
Copyright Notice: This document is for SYSPRO (Pty) Ltd internal usage only.

This document is provided outside of the SYSPRO development environment for informational purposes only. SYSPRO makes no warranties, express or implied, in this document. The information contained in this document is an overview of the intended functionality for SYSPRO and should not be interpreted to be a commitment on the final features, enhancements or scheduling.

## 1. DOCUMENT INFORMATION
User Azure DevOps profile ${name} ${surname}, ${role}

## 2. DOCUMENT HISTORY
<Version 1.0>

## 3. TABLE OF CONTENTS
1. DOCUMENT INFORMATION
2. 

## 4. GLOSSARY

## 5. PROGRAM LIST

## 6. DUPLICATING THE DEFECT IN IMP ENVIRONMENT (include Test Data)
6.1 ${DUPLICATION TEST ID}
6.2 ${DUPLICATION TEST ID}

## 7. TESTING THE CORRECTION IN TST ENVIRONMENT (include Test Data)
7.1 ${TEST CASE ID}
7.2 ${TEST CASE ID}

## 8. REGRESSION TESTS (include Test Data)
8.1 ${REGRESSION TEST ID}
8.2 ${REGRESSION TEST ID}

## 9. AUTOMATION TESTS (PLAYWRITE TYPESCRIPT) (include Test Data)
9.1 ${AUTOMATION TEST ID}
9.2 ${AUTOMATION TEST ID}